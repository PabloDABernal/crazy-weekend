...
**Nota para Architect y para Coordinator (importante)**: `game-design.md` → "Calendario de jornada — horarios escalonados" todavía contiene la frase *"Como máximo 2 partidos solapados (en curso) a la vez"* como regla dura, y la "Pregunta de diseño abierta" #3 pregunta si ese techo es fijo o puede subir en fases avanzadas. El Director Creativo confirmó ahora que **no debe existir techo fijo** en absoluto (ni de 2 ni de otro número), y que ciertas jornadas especiales deben poder concentrar todos los partidos a la vez. Esta historia se especifica ya con la versión corregida confirmada por el Director; queda pendiente que **Game Designer actualice esas 1-2 frases en `game-design.md`** antes de que Architect tome el documento como spec definitiva, para no diseñar sobre un dato desactualizado.
...
**Nota para Architect y Coordinator (importante)**: `game-design.md` → "Boleto vivo y feedback de resolución → resolución" todavía describe la resolución como *"una confirmación seca del ingreso"* sin fiesta ("un ganado no es una fiesta de confeti"), y la "Pregunta de diseño abierta" #4 deja sin cerrar si el feedback debe ser minimal desde la run 1 o más dopaminérgico en fases tempranas. El Director confirmó ahora que la dopamina debe ser **alta siempre**, sin condicionarla a fase narrativa — esto contradice el tono "seco/sin fiesta" descrito hoy en el documento. Esta historia se especifica ya con la versión confirmada por el Director; queda pendiente que **Game Designer actualice esas frases en `game-design.md`** antes de que Architect lo tome como spec definitiva.
