...
**Nota para Architect**: el texto exacto de la pantalla (contenido narrativo) es responsabilidad de Game Designer/narrative, no de esta historia ni de Architect; esta historia solo fija el disparo, el dato (qué día está vacío, qué día es el siguiente con partidos) y el contrato de UI (pantalla breve + avance automático), no la redacción final ni el diseño visual detallado.

**Texto narrativo (Game Designer, resuelto)**:
- **Fase 1-2 (runs 1-12, "superficie normal"/"grietas") — texto fijo por defecto**: *"Hoy no hay partidos. La jornada se concentra en [día]."* Tono de aviso de casa de apuestas real (el mismo registro frío/funcional que el resto de la interfaz de mercados) — no es un evento narrativo marcado, es información operativa. `[día]` se resuelve con el nombre del próximo día con partidos que ya calcula esta historia (ej. "domingo").
- **Fase 3-4 (runs 13+, "deterioro abierto"/"ruptura") — variante opcional**: *"Hoy no hay nada que ofrecerte. Todo está guardado para [día]."* Mismo dato funcional (el día), pero el registro se desliza levemente hacia el "casi te habla" ya establecido en los comentarios de partido y en el feedback de resolución de esas fases — sin romper el tono frío de la interfaz de mercados, coherente con `game-design.md` → "Arco narrativo" (fase 3: "el comentarista parece saber que el jugador está jugando"). Aporta valor real aquí porque esta pantalla, al ser un vacío total de contenido, es un lugar natural para que el deterioro se note por contraste (silencio que empieza a sentirse deliberado, no solo un aviso administrativo).
- Si Architect prefiere no diferenciar por fase para esta historia pequeña, el texto de fase 1-2 es válido como único texto fijo para todo el rango de runs; la variante de fase 3-4 es mejora, no bloqueante.
- Ambos textos respetan las reglas duras de usabilidad ya fijadas en `game-design.md` (sin solapamientos, saldo/hora siempre visibles) y no introducen ningún gate de apuesta ni input obligatorio — son puramente informativos, coherente con el criterio de éxito ya fijado arriba (avance automático).
