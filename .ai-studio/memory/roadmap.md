...
El juego se puede jugar de principio a fin como bucle de fin de semana que encadena runs con una transición real entre ellas: pantalla de inicio con intro narrativa, tutorial integrado de la primera apuesta, jornada del viernes con partidos generados por una liga ficticia simple (20 equipos) con horarios de kickoff escalonados por partido (sin techo fijo de partidos solapados, con soporte a jornadas especiales que concentran partidos), simulación por ticks de 15 minutos con panel informativo y comentarios (sin visual de partido), los 6 mercados de apuesta del MVP (1X2, goles 2.5, primer goleador, ambos marcan, tarjetas, faltas) con probabilidades, cuotas y ganancia potencial en rango que se recalculan y se retiran/aparecen según el minuto de partido, interfaz de apuestas tipo casa de apuestas fría con saldo, cuota/ganancia potencial visible antes de confirmar, y boleto vivo (estado en tiempo real de apuestas abiertas + feedback enérgico de resolución, sin condicionarlo a fase narrativa), cierre de run (domingo con dinero > 0 = victoria de run; 0$ en cualquier tick = derrota, "Ya es lunes"), y una **fase inter-run mínima** entre una run y la siguiente: resumen de lunes (qué pasó en el fin de semana, qué se desbloqueó) más una única decisión conversacional simplificada con beneficio de dinero extra (ver Historia E.6). Ver `backlog.md` → Épicas D (simulación de liga/partidos, incluyendo D.6 horarios escalonados y D.7 mercados por minuto) y E (UI de apuestas, interacción de tick, cuota/boleto vivo — E.7/E.8/E.9 — y fase inter-run mínima). Implementa formalmente el contrato `BettingTickService` que `_arquitectura-base.md` (sección 4) dejaba pendiente.

**Prioridad interna confirmada por el Director Creativo** para las historias añadidas tras el cierre del diseño del playtest run 1: (1) cuota/payout visible + boleto vivo con feedback de resolución (E.7, E.8), (2) horario escalonado de partidos (D.6), (3) mercados que varían/se restringen según el minuto de partido (D.7, E.9).
